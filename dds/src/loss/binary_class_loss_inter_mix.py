...
@register_criterion("binary_class_loss_inter_mix", dataclass=BinaryClassConfig)
class BinaryClassCriterion(FairseqCriterion):
    ...
    def forward(self, model, sample, reduce=True):
        ...
        logits, labels_pseudo = model.forward_inter_mix(**input)
        ...
        def bceloss(x, y):
            eps = 1e-6
            return -torch.mean(y * F.logsigmoid(x + eps) + (1 - y) * F.logsigmoid(1 - x + eps))
        
        # pdb.set_trace()
        loss = bceloss(logits, labels_pseudo[:, 1])
        ...
        logging_out = {
            ...
            "labels": labels_pseudo.data
        }

        return loss, sample_size, logging_out

    def forward_eval(self, model, sample, reduce=True):

        assert (hasattr(model, 'classification_heads')
                and self.classification_head_name in model.classification_heads)
        
        input = self.build_input(sample, self.classification_head_name)
        logits = model.forward_inter_mix_eval(**input)
