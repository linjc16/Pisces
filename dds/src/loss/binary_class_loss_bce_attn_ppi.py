...
@register_criterion("binary_class_loss_bce_attn_ppi", dataclass=BinaryClassConfig)
class BinaryClassBCECriterion(FairseqCriterion):
    ...
    def forward(self, model, sample, reduce=True):
        ...
        logits = torch.sigmoid(logits).mean(dim=1)

        labels = model.get_targets(sample['label'], None).view(-1).type_as(logits)
        ...
        loss = BCE_loss(logits, labels.long())
        # loss = - ((1 - labels) * torch.log(1 - logits + 1e-8) - labels * torch.log(logits + 1e-8)).mean()
        # print(loss)
        pos_preds = pos_logits.detach()
        neg_preds = neg_logits.detach()
