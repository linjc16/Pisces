...
@register_criterion("binary_class_loss_bce_dv_v2", dataclass=BinaryClassConfig)
class BinaryClassBCECriterion(FairseqCriterion):
    ...
    def forward(self, model, sample, reduce=True):
        ...
        loss += self.aux_alpha * loss_fn(logits_aux.squeeze(), labels.type_as(logits))
        
        reg_loss_fn = MSELoss()
        loss += self.scores_alpha * reg_loss_fn(F.logsigmoid(logits), F.logsigmoid(logits_aux))
