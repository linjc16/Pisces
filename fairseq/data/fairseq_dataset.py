    ...
    def batch_by_weight(
        ...
        def _get_labels(self):
            ...
            for i in range(len(self)):
                labels.append(self[i]['target'].item())
            return labels
        import pandas as pd
        # distribution of classes in the dataset
        df = pd.DataFrame()
        df["label"] = _get_labels(self)
        df.index = indices

        df.to_csv('S2_labels.csv')
        ...
        batch_samples = torch.multinomial(weights, num_samples, replacement=True)
        batch_sampler = torch.split(batch_samples, max_sentences)


        
        batch_sampler = [item.numpy() for item in batch_sampler]
