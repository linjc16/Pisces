...
class PairDeeperGCN(nn.Module):
    ...
    def pool_substructure(self, h, batch):
        node_fea = F.dropout(h, p=self.dropout, training=self.training)
        graph_fea = self.pool(node_fea, batch)
        return graph_fea

    def forward(self, graph, masked_tokens=None, features_only=False, perturbed=False):
        x = graph.x
        ...
        for layer in range(1, self.num_layers):
            ...
            if layer > self.num_layers - 4:
                graph_fea_list.append(self.pool_substructure(h, batch))
            h = F.dropout(h, p=self.dropout, training=self.training)
            h = self.gcns[layer](h, edge_index, edge_emb)
            h = h + residual
            if perturbed:
                random_noise = torch.rand_like(h).to(h.device)
                # h += torch.sign(h) * F.normalize(random_noise, dim=-1) * 0.05 / self.num_layers
        h = self.norms[0](h)
        h = self.activation_fn(h)
        node_fea = F.dropout(h, p=self.dropout, training=self.training)
        ...
        return graph_fea_list, (graph_fea, node_fea), atom_pred
...
#         self.gcns = nn.ModuleList()
#         self.norms = nn.ModuleList()
#         self.inter_encoders = nn.ModuleList()
#         self.activation_fn = utils.get_activation_fn(getattr(args, 'gnn_activation_fn', 'relu'))
...
#                 ))
#             self.norms.append(get_norm_layer(self.norm, self.embed_dim))
#             self.inter_encoders.append(nn.Sequential(
...
#         return h, edge_index, edge_emb, batch

#     def graph_inner_by_step(self, h, edge_index, edge_emb, layer_idx):
#         residual = h
#         h = self.norms[layer_idx](h)
#         h = self.activation_fn(h)
#         h = F.dropout(h, p=self.dropout, training=self.training)
#         h = self.gcns[layer_idx](h, edge_index, edge_emb)
#         h = h + residual

#         return h

#     def graph_output(self, h, batch, masked_tokens, features_only):
#         h = self.norms[0](h)
#         h = self.activation_fn(h)
#         node_fea = F.dropout(h, p=self.dropout, training=self.training)
...
#     def forward(self, graphA, graphB, masked_tokens=None, features_only=False):

#         h_A, edge_index_A, edge_emb_A, batch_A = self.graph_init(**graphA)
#         h_B, edge_index_B, edge_emb_B, batch_B = self.graph_init(**graphB)

        
#         for layer in range(1, self.num_layers):
#             h_A = self.graph_inner_by_step(h_A, edge_index_A, edge_emb_A, layer)
#             h_B = self.graph_inner_by_step(h_B, edge_index_B, edge_emb_B, layer)

#             h_inter = torch.cat([h_A, h_B], dim=-1)
#             h_inter = self.inter_encoders[layer](h_inter)

#             h_A += h_inter
#             h_B += h_inter

#         return{
#             'A': self.graph_output(h_A, batch_A, masked_tokens, features_only),
#             'B': self.graph_output(h_B, batch_B, masked_tokens, features_only)
#         }
