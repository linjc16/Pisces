...
def predicting(model, device, drug1_loader_test, drug2_loader_test):
    ...
    with torch.no_grad():
        for data in zip(drug1_loader_test, drug2_loader_test):
            ...
            data1 = data1.to(device)
            data2 = data2.to(device)
            output = model(data1, data2, data1.y.view(-1, 1))
...
for i in range(fold_num):
    ...
    checkpoint = torch.load("/data/linjc/dds/baselines/DeepDDS/data_leave_comb/results/GCNNet(DrugA_DrugB)0--model_train_fold0.model")
    ...
    for epoch in range(1):
        ...
        np.save('T_DeepDDS.npy', T)
        np.save('S_DeepDDS.npy', S)
        np.save('Y_DeepDDS.npy', Y)
        # T is correct label
        # S is predict score
        # Y is predict label
